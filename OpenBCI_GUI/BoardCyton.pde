...
class Cyton {
  ...
  private final int fsHzSerialCyton = 500;  //sample rate used by OpenBCI board...set by its Arduino code
  ...
  public void setSampleRate_zxc() {
    if(sampleRate == 500){
        hub.sendCommand("~5");
        println("5Setting sample rate for Cyton to " + sampleRate + "Hz");
	}else if(sampleRate == 250){
		hub.sendCommand("~6");
        println("6Setting sample rate for Cyton to " + sampleRate + "Hz");
	}
    println("7Setting sample rate for Cyton to " + sampleRate + "Hz");
  }
